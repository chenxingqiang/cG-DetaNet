...
from Datanet.code.detanet_model.detanet import DetaNet
...
def get_parser():
    ...
    #######  DetaNet  #######
    detanet_parser = argparse.ArgumentParser(add_help=False,
                                             parents=[model_parser])
    detanet_parser.add_argument('--features', type=int,
                                help='Size of atom-wise representation '
                                     '(default: %(default)s)',
                                default=128)
    detanet_parser.add_argument('--interactions', type=int,
                                help='Number of regular DetaNet interaction '
                                     'blocks (default: %(default)s)',
                                default=9)
    detanet_parser.add_argument('--dense_layers', type=int,
                                help='Number of layers in the (atom-wise) dense '
                                     'output networks to predict next type and '
                                     'distances (default: %(default)s)',
                                default=5)
    detanet_parser.add_argument('--cutoff', type=float, default=10.,
                                help='Cutoff radius of local environment '
                                     '(default: %(default)s)')
    detanet_parser.add_argument('--num_gaussians', type=int, default=25,
                                help='Number of Gaussians to expand distances '
                                     '(default: %(default)s)')
    detanet_parser.add_argument('--max_distance', type=float, default=15.,
                                help='Maximum distance covered by the discrete '
                                     'distributions over distances learned by '
                                     'the model '
                                     '(default: %(default)s)')
    detanet_parser.add_argument('--num_distance_bins', type=int, default=300,
                                help='Number of bins used in the discrete '
                                     'distributions over distances learned by '
                                     'the model(default: %(default)s)')

    detanet_parser.add_argument('--maxl', type=int, default=3,
    ...
    train_subparsers.add_parser('detanet', help='Detanet help',
                                parents=[train_parser, detanet_parser])

    eval_subparsers = subparser_eval.add_subparsers(dest='model',
                                                    help='Model-specific '
                                                         'arguments')
    eval_subparsers.required = True
    eval_subparsers.add_parser('detanet', help='DetaNet help',
                               parents=[eval_parser, detanet_parser])

    gen_subparsers = subparser_gen.add_subparsers(dest='model',
                                                  help='Model-specific '
                                                       'arguments')
    gen_subparsers.required = True
    gen_subparsers.add_parser('detanet', help='DetaNet help',
                              parents=[gen_parser, detanet_parser])
...
def get_model(args, conditioning_specification, parallelize=False):
    ...
    representation =\
        DetaNet(num_features=args.features,
                act='swish',
                maxl=args.maxl,
