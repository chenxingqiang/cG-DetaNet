...
def get_parser():
    ...
    train_parser.add_argument(
        "--use_embeddings_for_type_predictions",
    ...
    train_parser.add_argument(
        "--share_embeddings",
        help="Share embedding layers in DetaNet part and in "
    ...
    #######  DetaNet  #######
    detanet_parser = argparse.ArgumentParser(add_help=False, parents=[model_parser])
    detanet_parser.add_argument(
        "--features",
        type=int,
    ...
    detanet_parser.add_argument(
        "--interactions",
        type=int,
        help="Number of regular DetaNet interaction " "blocks (default: %(default)s)",
        default=9,
    )
    detanet_parser.add_argument(
        "--dense_layers",
    ...
    detanet_parser.add_argument(
        ...
        type=int,
        default=3,
        help="特征的最大度,建议设置为(default: %(default)s)",
    )
    ...
    train_subparsers.add_parser(
        "detanet", help="Detanet help", parents=[train_parser, detanet_parser]
    )
    ...
    eval_subparsers.add_parser(
        "detanet", help="DetaNet help", parents=[eval_parser, detanet_parser]
    )
    ...
    gen_subparsers.add_parser(
        "detanet", help="DetaNet help", parents=[gen_parser, detanet_parser]
    )
...
def get_model(args, conditioning_specification, parallelize=False):
    ...
    # get DetaNet layers for feature extraction
    representation = DetaNet(
        num_features=args.features,
        act="swish",
        maxl=args.maxl,
        num_block=args.interactions,
        radial_type="trainable_bessel",
        num_radial=args.num_gaussians,
        rc=args.cutoff,
        max_atomic_number=max_type + 3,
    )

    # spk.representation.DetaNet(n_atom_basis=args.features,
    ...
    model = spk.atomistic.AtomisticModel(
        representation,
        representation_conditioning_blocks + [out_module_type, out_module_dist],
    )
...
def train(args, model, train_loader, val_loader, device):
    """这段代码实现了原子分子模型的训练过程，
    包括设置优化器、损失函数、钩子等参数，以及开始训练过程并记录训练日志"""
...
def evaluate(args, model, train_loader, val_loader, test_loader, device):
    """
    设置了需要评估的指标，这些指标包括距离预测的KL散度和类型预测的KL散度。
    对训练集、验证集和测试集（根据参数args.split的设置）分别调用evaluate_dataset函数进行评估。
    将评估结果保存到名为evaluation.csv的CSV文件中。
    """
...
def evaluate_dataset(metrics, model, loader, device):
    """
    遍历数据加载器(loader)中的每个批次(batch)，将批次数据移到指定的设备(device)上。
    使用模型(model)对批次数据进行预测，并计算评估指标(metrics)。
    将每个指标的评估结果聚合并返回
    """
...
def generate(args, train_args, model, device, conditioning_layer_list):
    ...
    获取数据集的可用原子类型和相关信息。
    设置生成分子的数量、生成批次的大小等参数。
    设置条件信息：

    从参数中提取条件信息，例如指纹、化学成分等。
    构建条件信息的字典，并对缺失信息进行处理。
    生成分子：

    使用模型生成分子，并根据参数设置保存生成过程或未完成的分子。
    根据条件信息对生成的分子进行条件约束。
    打印生成进度和统计信息：

    根据设置的步长和阈值，打印生成进度。
    打印生成的分子数量和所花费的时间。
    如果设置了显示生成的分子，则显示生成的分子并打印相关统计信息。
    返回生成的分子结果：

    返回生成的分子结果字典，包含生成的分子结构和相关信息。
    ...
    if args.print_file:
        progress = lambda x, y: (
            print(f"Generated {x}.", flush=True)
            if x >= y
    ...
    if args.conditioning is not None:
        ...
        for cond_list in conds:
            ...
            if cond_name not in conditioning_layer_list:
                logging.info(
                    f"The provided model was not trained to condition on "
            ...
            if cond_name == "fingerprint":
                if not os.path.isfile("./data/qm9gen.db"):
                    logging.error(
                        f"could not find database with fingerprints at ./data/qm9gen.db!"
                    )
                ...
                with connect("./data/qm9gen.db") as conn:
                    if cond_vals[0].isdigit():
                        if "fingerprint_format" not in conn.metadata:
                            ...
                            logging.error(
                                f"please re-download data when training a model with fingerprints as condions!"
                            )
                            logging.error(f"stopping generation!")
                            return
                        fp = np.array(
                            conn.get(int(cond_vals[0]) + 1).data["fingerprint"],
                        ...
                        fp = np.array(
                            pybel.readstring("smi", cond_vals[0]).calcfp().fp,
                            dtype=conn.metadata["fingerprint_format"],
                        )
                        from collections import Counter

                        _ct = Counter(cond_vals[0].lower())
                        conditioning["_composition"] = np.array(
                            [_ct["c"], _ct["n"], _ct["o"], _ct["f"]]
                        )
                    conditioning["_fingerprint"] = np.unpackbits(
                        fp.view(np.uint8), bitorder="little"
                    )[None, ...]
            else:
    ...
    for block in conditioning_specification:
        ...
        for layer_name in layers:
            ...
            if layer_name == "composition":
                ...
                conditioning_extractors.update(
                    {
                        "_composition": lambda x, prob=p: (
                            torch.FloatTensor(get_composition(x, all_types)),
                            prob,
                        )
                    }
                )
                ...
                conditioning_extractors.update(
                    {"_" + layer_name: lambda x, prob=p, n=layer_name: (x.pop(n), prob)}
                )
...
def get_conditioning_nn(name, dataclass):
    ...
    if name == "fingerprint":
        ...
        layers = lambda embedding, **kwargs: AtomCompositionEmbedding(
            nn.Embedding(**embedding), **kwargs
        )
...
def main(args):
    ...
    if args.mode == "train":
        ...
        if args.pretrained_path is not None:
            ...
            if (
                ...
                # 如果模型目录下已经存在检查点，则警告用户会忽略预训练模型并使用最新检查点
                logging.info(
                ...
                if not os.path.exists(pretrained_chkpt_path):
                    ...
                    if len(chkpt_files) == 0:
                        # 如果预训练模型目录中没有检查点，则无法加载检查点，警告用户会从头开始训练
                        logging.warning(
                        ...
                        # 如果预训练模型目录中存在检查点，则复制最新检查点到模型目录下
                        epoch = max(
    ...
    conditioning_specification_path = os.path.join(
        args.modelpath, "conditioning_specification.json"
    )
    ...
    # else look for specification at path provided with the training arguments
        ...
        if os.path.isfile(args.conditioning_json_path):
            ...
            logging.error(
                f"The provided conditioning specification file "
    ...
    # process information
    load_additionally, conditioning_extractors, cond_layer_list = prepare_conditioning(
        conditioning_specification
    )
    # store specification of conditioning layers in model folder
    if (
        conditioning_specification is not None
        and conditioning_json_path != conditioning_specification_path
    ):
    ...
    if args.mode == "train" or args.checkpoint >= 0:
        model = get_model(
            train_args, conditioning_specification, parallelize=args.parallel
        ).to(device)
