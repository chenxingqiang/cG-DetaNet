...
class QM9gen(DownloadableAtomsData):
    ...
    def __init__(
        ...
        with connect(self.dbpath) as db:
            if db.count() <= 0:
                logging.error(
                    "Error: Data base is empty, please provide a path "
            ...
            if self.precompute_distances and "dists" not in db.get(1).data:
                logging.info(
                    "Caution: Existing data base does not contain "
            ...
            if "fingerprint" in self.load_additionally:
                if "fingerprint" not in db.get(1).data:
                    logging.error(
                        "Error: Fingerprints not found in the provided data "
            ...
            for prop in self.load_additionally:
                if prop not in db.get(1).data and prop not in self.properties:
                    logging.error(
                        f"Error: Unknown property ({prop}) requested for "
    ...
    def create_subset(self, idx):
        ...
        return type(self)(
            self.path, subidx, download=False, load_additionally=self.load_additionally
        )
    ...
    def get_properties(self, idx):
        ...
        for add_prop in self.load_additionally:
            if add_prop == "fingerprint":
                fp = np.array(
                    row.data[add_prop], dtype=self.db_metadata["fingerprint_format"]
                )
                ...
                relative_atomic_energy = energy_per_atom - self.energy_regression(
                    concentration
                )
    ...
    def _preprocess_qm9(self):
        ...
        if os.path.exists(raw_path):
            logging.info(
                f'Found existing list with indices of molecules in QM9 that are invalid at "{raw_path}".'
            ...
            try:
                ...
                logging.error("HTTP Error:", e.code, url)
                logging.info(
                    "CAUTION: Could not download pre-computed list, will assess "
        ...
        valence_list = np.array(
            [self.available_atom_types, self.atom_types_valence]
        ).flatten("F")
    ...
    def create_splits(self, num_train=None, num_val=None, split_file=None):
        ...
        if not os.path.exists(invalid_file_path):
            raise ValueError(
                f"Cannot find required file with indices of QM9 molecules that are invalid at {invalid_file_path}!"
            )
        ...
        if (
            len(removed_idx) != len(invalid_idx)
            or len(removed_idx.difference(invalid_idx)) != 0
        ):
            raise ValueError(
                f"Mismatch between the data base used to generate the provided split file and your local database. "
