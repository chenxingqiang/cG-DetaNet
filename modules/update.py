...
class Tensorproduct_Attention(nn.Module):
    def __init__(
        ...
        self.lq = o3.Linear(
            irreps_T, irreps_T, internal_weights=True, shared_weights=True
        )
        self.lk = o3.Linear(
            irreps_T, irreps_T, internal_weights=True, shared_weights=True
        )
        ...
        self.tp1 = o3.TensorProduct(
            irreps_T, irreps_T, irreps_scalar, instructions=intp1
        )
        self.tp2 = o3.TensorProduct(
            irreps_scalar, irreps_T, irreps_T, instructions=intp2
        )
    ...
    def forward(self, T, S):
        ...
        su, sd = torch.split(
            self.softmax(self.ls(s)),
            split_size_or_sections=[self.feature, self.feature],
            dim=-1,
        )
...
class Update(nn.Module):
    def __init__(self, num_features, act, irreps_mout, irreps_T, dropout=0.0):
        ...
        self.uattn = Tensorproduct_Attention(
            num_features=num_features, irreps_T=irreps_T, act=act
        )
        self.reset_parameters()
