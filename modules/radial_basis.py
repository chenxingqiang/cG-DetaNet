...
class Bessel_Function(nn.Module):
    ...
    def __init__(self, num_radial, rc, weighting=True, inital_beta=2.0):
        ...
        if weighting:
            self.register_parameter(
                "alpha",
                nn.Parameter(
                    torch.arange(0, num_radial, dtype=torch.float32).view(1, num_radial)
                ),
            )
            ...
            self.register_buffer(
                "alpha",
                torch.arange(1, num_radial + 1, dtype=torch.float32).view(
                    1, num_radial
                ),
            )
            self.beta = 1.0
    ...
    def forward(self, r, cutoff=None):
        ...
        rbf = (
            self.prefactor
            * torch.sin(self.alpha * torch.pi * r / self.rc)
            / (self.beta * r)
        )
...
class Radial_Basis(nn.Module):
    ...
    def __init__(
        ...
        radials = {
            "gaussian": Exp_Gaussian_function(
                num_radial=num_radial, exp_weighting=False
            ),
            "exp_gaussian": Exp_Gaussian_function(
                num_radial=num_radial, exp_weighting=True
            ),
            "bessel": Bessel_Function(num_radial=num_radial, rc=rc, weighting=False),
            "trainable_bessel": Bessel_Function(
                num_radial=num_radial, rc=rc, weighting=True
            ),
        }
