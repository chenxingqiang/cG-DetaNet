...
class DetaNet(nn.Module):
    def __init__(
        ...
        assert (
            num_features % attention_head == 0
        ), "attention head must be divisible by the number of features"
        self.scale = scale
        self.ref = atom_ref
        self.norm = norm
        ...
        if scalar_outsize != 0:
            self.sout = MLP(
                size=(num_features, num_features, scalar_outsize),
                act=act,
                dropout=dropout,
            )
    ...
    def grad_hess_ii(self, energy, posa, posb, create_graph=True):
        ...
        for i in range(3):
            gii = -grad(
                [f[:, i].sum()], [posb], create_graph=create_graph, retain_graph=True
            )[0]
            Hii[:, i] = gii
        return Hii
