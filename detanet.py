...
class DetaNet(nn.Module):
    ...
    device: device type, either torch.device('cuda') represents gpu or torch.device('cpu')  represents cpu
        ...
        irreps_T = o3.Irreps((num_features, (l, (-1) ** l))
        ...
        self.Embedding = Embedding(
            num_features=num_features, act=act, device=device, max_atomic_number=max_atomic_number)
        self.Radial = Radial_Basis(
            radial_type=radial_type, num_radial=num_radial, use_cutoff=use_cutoff)
        blocks = []
        # interaction layers
        ...
        if irreps_out is not None:
            ...
            self.tout = Equivariant_Multilayer(
                irreps_list=[irreps_T, irreps_mid, irreps_out], act=act)
        if scalar_outsize != 0:
            self.sout = MLP(size=(num_features, num_features,
                            scalar_outsize), act=act, dropout=dropout)
    ...
    def cal_p_tensor(self, z, pos, batch, outs, outt):
        ...
        ra = self.centroid_coordinate(z=z, pos=pos, batch=batch)
        ta = o3.spherical_harmonics(
            l=self.irreps_out, x=ra, normalize=False)*sa
    ...
    def grad_dipole(self, dipole, pos):
        ...
        for i in range(0, 3):
            dedipole[:, :, i] = - \
                grad([dipole[:, i].sum()], [pos], create_graph=True)[0]
        return dedipole
    ...
    def grad_polarzability(self, polars, pos):
        ...
        for i in range(0, 6):
            depolar[:, :, i] = - \
                grad([polars[:, i].sum()], [pos], create_graph=True)[0]
        return depolar
    ...
    def forward(self,
        ...
        S = self.Embedding(z)
        T = torch.zeros(size=(S.shape[0], self.vdim),
        ...
        sh = o3.spherical_harmonics(
            l=self.irreps_sh, x=rij/(r.view(-1, 1)), normalize=True, normalization="component")
        ...
        if self.out_type == 'scalar':
            ...
            out = self.cal_dipole(
                z=z, pos=pos, batch=batch, outs=outs, outt=outt)

        elif self.out_type == '2_tensor':
            out = self.cal_p_tensor(
                z=z, pos=pos, batch=batch, outs=outs, outt=outt)
        ...
        if self.grad == 'force':
            ...
            out = self.grad_polarzability(
                polars=out.reshape(-1, 3, 3), pos=pos)
