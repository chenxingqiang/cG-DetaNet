...
class Molecule:
    ...
    def get_fixed_connectivity(self, recursive_call=False):
        ...
        if np.sum(N_mat) < len(N_idcs) * 3 or np.sum(C_mat) < len(C_idcs) * 4:
            for cur in NC_idcs:
                ...
                if sum(con_mat[cur]) >= self.type_infos[type]["n_bonds"]:
                    continue
                CC_nbors = self._get_CN_neighbors(
                    cur, order_1=1, order_2=3, only_CC=True, include_CCC=True
                )
    ...
    def _get_CN_neighbors(
        self, idx, order_1=1, order_2=1, only_CC=False, include_CCC=False
    ):
        """
        ...
        if not only_CC:
            CN_nbors = np.where(
                np.logical_and(_numbers == 7, con_mat[C_nbors] >= order_2)
            )
            CN_nbors = [
                (C_nbors[CN_nbors[0][i]], CN_nbors[1][i])
                for i in range(len(CN_nbors[0]))
            ]
        if type == 7 or include_CCC:  # for N atoms, also add C-C neighbors
            CC_nbors = np.where(
                np.logical_and(_numbers == 6, con_mat[C_nbors] >= order_2)
            )
            CC_nbors = [
