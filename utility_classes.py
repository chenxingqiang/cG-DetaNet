...
class Molecule:
    ...
    def __init__(self, pos, atomic_numbers, connectivity_matrix=None,
        ...
        # was causing problems with other scripts if imported outside of this class
        # import openbabel as ob
        # import pybel
        # from rdkit import Chem
    ...
    def get_fixed_connectivity(self, recursive_call=False):
        ...
        if (np.sum(N_mat) < len(N_idcs) * 3 or np.sum(C_mat) < len(C_idcs) * 4) \
            ...
            for cur in NC_idcs:
                ...
                if sum(con_mat[cur]) >= self.type_infos[type]['n_bonds']:
                    continue
                CN_nbors = self._get_CN_neighbors(cur, order_1=1, order_2=2)
        ...
        # increase total number of bonds by transferring the strength of a
        # triple C-C bond to two neighboring bonds, if the involved atoms
        ...
        if (np.sum(N_mat) < len(N_idcs) * 3 or np.sum(C_mat) < len(C_idcs) * 4):
            for cur in NC_idcs:
                ...
                if sum(con_mat[cur]) >= self.type_infos[type]['n_bonds']:
                    continue
                CC_nbors = self._get_CN_neighbors(cur, order_1=1, order_2=3,
                                                  only_CC=True, include_CCC=True)
                for nbor_1, nbor_2 in CC_nbors:
                    nbor_2_nbors = np.where(con_mat[nbor_2] == 1)[0]
                    for nbor_2_nbor in nbor_2_nbors:
                        nbor_2_nbor_type = self.numbers[nbor_2_nbor]
                        if (np.sum(con_mat[nbor_2_nbor]) <
    ...
    def _get_CN_neighbors(self, idx, order_1=1, order_2=1,
                          only_CC=False, include_CCC=False):
        '''
        For a focus atom of type K returns indices of atoms C (carbon) and N (nitrogen)
        on two-step paths of the form K-C-N (and K-C-C only if K is nitrogen since one
        atom needs to be nitrogen, except if include_CCC is set True). The required
        bond order on the path can be constraint using the parameters order_1 and
        order_2.

        Args:
            idx (int): the index of the focus atom from which paths are examined
            order_1 (int, optional): the minimum bond order that neighbors must share
                with the focus atom in order to be added to the results (allows to
                constrain the paths to e.g. K=C-N or K=C-C with order_1=2, default: 1)
            order_2 (int, optional): the minimum bond order that second degree
                neighbors must share with the first degree neighbors in order to be
                added to the results (allows to constrain the paths to e.g. K-C=N or
                K-C=C with order_2=2, default: 1)
            only_CC (bool, optional): include only K-C-C paths (i.e. ignore K-C-N,
                default: False)
            include_CCC (bool, optional): also include K-C-C paths if K is carbon
                (default: False)
        ...
        CC_nbors = []
        if not only_CC:
            CN_nbors = np.where(np.logical_and(_numbers == 7,
                                               con_mat[C_nbors] >= order_2))
            CN_nbors = [(C_nbors[CN_nbors[0][i]], CN_nbors[1][i])
                        for i in range(len(CN_nbors[0]))]
        if type == 7 or include_CCC:  # for N atoms, also add C-C neighbors
            CC_nbors = np.where(np.logical_and(_numbers == 6,
                                               con_mat[C_nbors] >= order_2))
            CC_nbors = [(C_nbors[CC_nbors[0][i]], CC_nbors[1][i])
                        for i in range(len(CC_nbors[0]))]
        return CN_nbors + CC_nbors
...
class ConnectivityCompressor():
    '''
    用于压缩和解压连接性矩阵的实用类。
    它提供了方法来将连接性矩阵压缩为较小的格式，并且可以从压缩格式中还原出完整的连接性矩阵。
    这种压缩和解压缩的过程对于处理大量连接性数据时可以节省存储空间，
    并且方便处理连接性矩阵的传输和存储
    ...
    def compress(self, connectivity_matrix):
        ...
        for i in np.unique(smaller):  # store indices per bond order > 0
            if i > 0:
                d[float(i)] = np.where(smaller == i)[0]
        return d
...
class IndexProvider():
    '''
    根据提供的统计信息对大量分子进行过滤，以获取符合特定结构要求的分子。
    用户可以通过提供的统计信息和选择字符串来指定过滤条件，例如过滤具有特定数量碳原子的分子。
    这个类提供了灵活的过滤功能，可以根据用户需求进行定制化的分子筛选操作。
...
class ProcessQ(Process):
    '''
    继承自 multiprocessing.Process 的类，用于在多进程环境中运行指定的函数，
    并将结果放入一个给定的 multiprocessing.Queue 对象中，
    以便主进程可以轻松获取函数的返回结果。这个类主要用于实现多进程并行计算，
    可以有效地利用多核处理器资源，提高程序的运行效率。
