...
class KLDivergence(Metric):
    ...
    Args:
        ...
        inverse_mask (bool, optional): whether the mask needs to be inverted prior to
            ...
            KLDivergence 类的作用是在训练或评估过程中计算模型预测结果与真实标签之间的平均KL散度，
            用于评估模型在概率分布方面的准确性
    """
...
class EmbeddingMultiplication(nn.Module):
    ...
    Args:
        ...
        EmbeddingMultiplication 类的作用是将原子类型的嵌入与表示相乘，
        用于增强原子类型信息对于模型的影响，
...
class GaussianExpansion(nn.Module):
    ...
    def forward(self, property):
        ...
        对输入的属性值进行高斯扩展，以便更好地表示和利用属性信息，
        从而在深度学习模型中提升对属性的处理能力和预测效果
        """
