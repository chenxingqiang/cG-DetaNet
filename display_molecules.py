...
def print_indices(idcs, name="", per_line=10):
    ...
    Args:
        ...
        name (str, optional): the selection string that was used to obtain the indices
        per_line (int, optional): the number of indices that are printed per line (
            default: 10)
    """
    biggest_count = len(str(len(idcs)))
    format_count = f">{biggest_count}d"
    biggest_original_idx = len(str(max(idcs)))
    new_line = "\n"
    format = f">{biggest_original_idx}d"
    str_idcs = [
        f"{i+1:{format_count}}: {j:{format}}  "
        + (new_line if (i + 1) % per_line == 0 else "")
...
if __name__ == "__main__":
        ...
        if not os.path.isfile(args.data_path):
            ...
            if not os.path.isfile(stats_path):
                ...
                index_provider = IndexProvider(
                    stats_dict["stats"], stats_dict["stat_heads"]
                )
    ...
    if args.train_data_path is not None:
        if not os.path.isfile(args.train_data_path):
            ...
            if not os.path.isfile(stats_path) and len(train_selections) > 0:
                ...
                train_index_provider = IndexProvider(
                    train_stats_dict["stats"], train_stats_dict["stat_heads"]
                )
    ...
    if (
        (len(gen_selections) == 0)
        and not (args.train or args.test or args.novel)
        and args.data_path is not None
    ):
    ...
    if (
        (len(train_selections) == 0)
        and not (args.train or args.test)
        and args.train_data_path is not None
    ):
                ...
                with connect(args.train_data_path) as con:
                    for query in db_queries:
                        ...
                        if head not in con.get(1).data:
                            print(
                                f"Entry {head} not found for molecules in the "
                            ...
                            num = float(
                                train_index_provider.num_re.search(condition).group(0)
    ...
    if args.train:
        ...
        if len(idcs) == 0:
            ...
            view_ase(
                _ats, f"generated molecules (matching train structures)", args.block
            )
            if args.export_to_dir is not None:
    ...
    if args.test:
        ...
        if len(idcs) == 0:
            ...
            view_ase(
                _ats, f"generated molecules (matching test structures)", args.block
            )
            if args.export_to_dir is not None:
