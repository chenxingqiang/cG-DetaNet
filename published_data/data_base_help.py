...
def get_parser():
    ...
    main_parser.add_argument(
        "data_base_path", type=str, help="Path to data base (.db file) with molecules."
    )
...
if __name__ == "__main__":
    ...
    with connect(args.data_base_path) as con:
        if con.count() == 0:
            ...
            print(
                f'Error: The metadata of data base at "{path}" does not contain the field "info"!'
            )
            print(
                f"However, this is the data stored for the first molecule:\n{con.get(1).data}"
            )
            sys.exit(0)
        ...
        print(
            f"You can visualize the molecules in the data base with ASE from the command line by calling:"
        )
