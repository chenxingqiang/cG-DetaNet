...
def _process_duplicates(dups, n_mols):
    ...
    for duplicate, original in dups:
        ...
        change_idcs = np.array([duplicate] + wrongly_assigned_originals, dtype=int)
        duplicating[change_idcs] = original
...
def collect_fingerprints_and_cans(mols):
    """
    Collects the bits set in fingerprints and canonical SMILES strings (for normal and
    mirrored versions) of the provided molecules if they are valid (otherwise they
    are set to None).
    ...
    Returns:
        dict (str->[int/str]): dictionary containing bits set in the fingerprints and
            the canonical SMILES strings
    """
    fps_bits = [None for _ in range(len(mols))]
    cans = [None for _ in range(len(mols))]
    mirror_cans = [None for _ in range(len(mols))]
    for i, mol in enumerate(mols):
        fps_bits[i] = mol.get_fp_bits()
        cans[i] = mol.get_can()
        mirror_cans[i] = mol.get_mirror_can()
    return {
        "fingerprint_bits": fps_bits,
        "canonical_smiles": cans,
        "mirrored_canonical_smiles": mirror_cans,
    }
...
if __name__ == "__main__":
    ...
    if "all" not in args.store:
        ...
        for key in res:
            ...
            # store addational information if desired
